...
Level.prototype.obstacleAt = function (pos, size) {
	...
	//Consider sides of level as walls
	if (xStart < 0 || xEnd > this.width || yStart < 0 || yEnd > this.height)
		return "wall";
	
	//Check each grid pos starting at yStart, xStart
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  //Find if theres an obstacle at newPos
  var obstacle = level.obstacleAt(newPos, this.size);
	
  //Move if there's not a wall
  if (obstacle != "wall")
	  this.pos = newPos;
};
...
Player.prototype.moveY = function(step, level, keys) {
	...
	// The floor is also an obstacle -- only allow players to jump
	// if they are touching some obsacle 
	if (obstacle) {
